...
(lib._728x90 = function(mode,startPosition,loop) {
	...
	this.frame_0 = function() {
		...
		function cubeAnimation() {
			...
			dom_overlay_container.addEventListener('mouseenter', startAnimation);
		...
		function squareAnimation() {
			...
			dom_overlay_container.addEventListener('mouseenter', restartAnimation);
		...
		function setData(data, cam) {
			...
			if(data) {
				...
				function addContent(assets) {
					...
					ad.setText(ad.root.headlineText, data['headlineText'].split('|')[0], 'center');
					...
					onAdClick = function(e) { amo.onDynAdClick(data.feedData, "CLICK", data.url, data.dataId, (new Date()).getTime()); }
					// if(ad.root) {
